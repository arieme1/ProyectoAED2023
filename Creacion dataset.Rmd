...
for (año in años) {
  ruta_carpeta <- paste0("data/Variables/", año)
  archivos_climaticos <- list.files(ruta_carpeta, pattern = ".*.csv")
  
  datos_año <- data.frame(Indicativo = estaciones$INDICATIVO)
  lista_año <- list()
  ...
  for (archivo in archivos_climaticos) {
    ...
    lista_año[[ruta_completa_archivo]] <- variable
    ...
    datos_año <- merge(datos_año, datos_variable, by = "Indicativo", sort = FALSE, all.x = TRUE, all.y = TRUE)
  }
  indice_menor_filas <- which.min(sapply(lista_año, nrow))
  dataframe_menor_filas <- lista_año[[indice_menor_filas]]
  Indicativos<- dataframe_menor_filas$Indicativo 
  datos_año <- datos_año[datos_año$Indicativo %in% Indicativos, ]
  assign(paste0("datos_", año), datos_año)
  View(datos_año)
}
